...
> ```
>
> To capture the LRO operation id from `az rest`, use `--verbose` and parse the `x-ms-operation-id` header from stderr, then poll `https://api.fabric.microsoft.com/v1/operations/{operationId}` — see [authoring mode § LRO Header Capture](../authoring.md#lro-header-capture-with-az-rest). Do not poll the raw `Location` header (it redirects to an `analysis.windows.net` host and fails auth).
