<!-- Mode reference for the `dataflows-cli` skill. Loaded on demand from `skills/dataflows-cli/SKILL.md` when the request matches the `consumption` mode. -->

> **MODE-CRITICAL NOTES (consumption mode)**
> 1. To find a dataflow by name: list all dataflows in the workspace and filter by `displayName` client-side — there is no server-side name filter
> 2. `getDefinition` is a **POST**, not GET — even though it reads data
...
> `dataflows-cli authoring mode` only as a future handoff. Do not invoke that skill,
...
>
> **Connection-capability boundary:** Reading a saved dataflow's existing
> connection bindings is in scope. Discovering connection connector options,
> required parameters, or supported credential types for Dataflow Gen2 setup is
> an authoring/configuration task, including read-only lookups of
> `supportedConnectionTypes`.

# dataflows-cli consumption mode — Dataflows Gen2 Consumption via CLI
...
| Quick Reference One-Liners | [consumption-cli-quickref.md](consumption/consumption-cli-quickref.md) | `az rest` one-liners for all consumption ops |
| Discovery Patterns | [discovery-queries.md](consumption/discovery-queries.md) | Definition decoding, parameter extraction, connection analysis |
| Script Templates | [script-templates.md](consumption/script-templates.md) | Copy-paste bash and PowerShell templates |
| Preview Data Visualization | [chart-visualization.md](consumption/chart-visualization.md) | Render `executeQuery` results as ASCII line/bar/pie charts (dependency-free) |
...
Run these in order to fully explore a workspace's dataflows. See [references/discovery-queries.md](consumption/discovery-queries.md) for extended patterns.
...
6. **Present** → Summarize findings or generate a reusable script (see [script-templates.md](consumption/script-templates.md)).
...
> **Failures return HTTP 200**: `executeQuery` returns `200 OK` with `application/vnd.apache.arrow.stream` even when the underlying source query fails (Kusto SEM0100, T-SQL syntax error, missing column, etc.). The error is embedded inside the stream's `PQ Arrow Metadata` section as `{"Error":"..."}` — see [dataflows-cli authoring mode § mashup-preview.md → Detecting failures inside the Arrow body](authoring/mashup-preview.md#detecting-failures-inside-the-arrow-body) for detector snippets. Naive HTTP-status checks will treat failures as success.

> **Intent split (canonical executeQuery reference is [mashup-preview.md](authoring/mashup-preview.md))**: the same `executeQuery` endpoint serves two distinct intents. This skill covers the **consumption** intents:
...
> **Body shape**: send a **flat object** with a top-level `QueryName` (field name is case-insensitive on the wire; PascalCase canonical). The `{"queries":[{...}]}` array shape always returns `400 DataflowExecuteQueryError: Invalid query name` regardless of inner casing. A wrong `QueryName` value returns `QueryNotFound` (different error code). See [dataflows-cli authoring mode § mashup-preview.md → Request body](authoring/mashup-preview.md#custommashupdocument-format).

> **`TimedOut` recovery for heavy persisted queries**: if the persisted `shared <Q>` member is too expensive for the server-side `executeQuery` budget (~90s observed; surfaces as `400 DataflowExecuteQueryError` → `InternalErrorCode: EvaluationError, Message: Evaluation result error code: TimedOut`), do **not** retry the same body — it is deterministic. Instead, re-issue with `customMashupDocument` set to the full persisted `mashup.pq` (decoded via Example 2) **plus** an appended probe `shared __PreviewCap = Table.FirstN(<Q>, 10);` and `QueryName: "__PreviewCap"`. The cap pushes down for most connectors (Kusto, SQL, OData, web tables) so the source returns only the sample. See [dataflows-cli authoring mode § mashup-preview.md → Observed `InternalErrorCode` values](authoring/mashup-preview.md#b-non-200-with-errorresponse-json-envelope).
...
| **Side-effect disclosure** | This is a **read-only** skill — never imply mutation, and **refuse** any create/update/delete request (route to `dataflows-cli authoring mode`). |
